class Dhl::Pickup::Piece
  ...
  def initialize(options = {})
    ...
    if options[:weight] && options[:weight] > 0
      ...
      raise Dhl::Pickup::OptionsError, required_option_error_message(:weight)
    end
    ...
    if options[:width] || options[:height] || options[:depth]
      [ :width, :height, :depth].each do |req|
        if options[req].to_f > 1
          ...
          raise Dhl::Pickup::OptionsError, required_option_error_message(req)
        end
      end
    end
...
private
  ...
  def required_option_error_message(field)
    ":#{field} is a required for Dhl::Pickup::Piece. Must be nonzero integer or float."
  end
end